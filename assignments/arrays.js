...
let lastCar = inventory.car_make[inventory.length - 1] + inventory.car_model[inventory.length - 1]
...
for (i = 0; i < inventory.length; ++i){
    carModels.push(inventory.car_model[i]);
    carModels.sort();
}
...
for (i = 0; i < inventory.length; ++i) {
    if (inventory.car_model[i] === "BMW" || inventory.car_model[i] === "Audi") {
        BMWAndAudipush(inventory[i])
    }
